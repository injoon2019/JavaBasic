...
class Friend{
	String name;
	Optional<Company> cmp;	//null일 수 있음
...
class Company{
	String cName;
	Optional<ContInfo> cInfo;	//null일 수 있음
...
class NullPointerCaseStudy3{
	public static void showCompAddr(Optional<Friend> f){
		String addr = f.flatMap(Friend::getCmp).flatMap(Company::getCInfo).flatMap(ContInfo::getAdrs).orElse("There's no address information");
